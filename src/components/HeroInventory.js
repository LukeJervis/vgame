...
const HeroInventory = () => {
    ...
    const { heroInventoryStore: { heroWeaponInv, heroArmourInv, heroItemsInv, heroPetSlotsArray, handleSell } } = useRootStore()
    ...
    const weaponInv = heroWeaponInv.map(heroEquipment => 
        <div key={Math.random().toString(36)} className='HeroInventory__equipment'>
            ...
            <div className='HeroInventory__buttonContainer'>
                <button className='HeroInventory__button' onClick={() => sellItemsHandler(heroEquipment, 'one')}>S</button>
                <button className='HeroInventory__button' onClick={() => heroWeaponEquipHandler(heroEquipment)} key={heroEquipment.id}>E</button>
            </div>
        </div>
    )
    ...
    const itemInv = heroItemsInv.map(heroEquipment => 
        <div key={Math.random().toString(36)} className='HeroInventory__equipment'>
            ...
            <div className='HeroInventory__buttonContainer'>
                <button className='HeroInventory__button' onClick={() => sellItemsHandler(heroEquipment, 'one')}>S</button>
                <button className='HeroInventory__button' onClick={() => sellItemsHandler(heroEquipment, 'all')}>S/A</button>
                <div className='HeroInventory__count' key={Math.random().toString(36)}>{heroEquipment.count}</div>
            </div>
        </div>
    )
    ...
    const heroPetCount = heroPetSlotsArray.map(heroPet =>
        <div key={Math.random().toString(36)} className='HeroInventory__pets'>
            ...
            <div className='HeroInventory__buttonContainer'>
                <button className='HeroInventory__button' onClick={() => heroPetEquipHandler(heroPet)} key={Math.random().toString(36)}>E</button>
                <button className='HeroInventory__button' onClick={() => unequipPet()} key={Math.random().toString(36)}>U</button>
            </div>
        </div>    
    )
