        ...
        if (monsterLoot[i].type === "item") {
            ...
            if (spin <= luck) {
                if (randomNumber(0, monster.statChance) <= 10) {
                    damageBonus = randomNumberToTwo(
                        monsterLoot[i].statLow,
                        monsterLoot[i].statHigh
                    );
                    prefixNum++;
                }
                if (randomNumber(0, monster.statChance) <= 10) {
                    speedBonus = randomNumberToTwo(
                        monsterLoot[i].statLow,
                        monsterLoot[i].statHigh
                    );
                    prefixNum++;
                ...
                if (randomNumber(0, monster.statChance) <= 50) {
                    moneyIncrease = randomNumberToTwo(
                        monsterLoot[i].statLow,
                        monsterLoot[i].statHigh
                    );
                    prefixNum++;
                ...
                let droppedWeapon = {
                    ...
                    moneyIncrease: moneyIncrease
                        ? Math.round(moneyIncrease)
                        : 0,
                    lifeSteal: lifeSteal ? lifeSteal / 2 : 0,
                    damage: damageBonus
                        ? +damageBonus + +monsterLoot[i].damage
                        : monsterLoot[i].damage,
