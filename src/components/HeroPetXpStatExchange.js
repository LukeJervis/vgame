...
const HeroPetXpStatExchange = () => {
  const {
    countStore: { experience, spendexperience },
  } = useRootStore();
  const {
    heroStatsStore: { handlePetStatBuy, equipedPet },
  } = useRootStore();
      ...
      return (
        <div className="heroPetXpStatExchange__container">
          <div className="heroPetXpStatExchange__health">
            ...
            <button className="heroPetXpStatExchange__strength" onClick={() => petStatBuy(equipedPet, healthBuy)}>
              Buy {Math.floor(equipedPet.healthStatCost * equipedPet.healthStatMulti)}XP
            </button>
          </div>
          <div className="heroPetXpStatExchange__strength">
            Strength: {equipedPet.strength}
            <button
              className="heroPetXpStatExchange__strength--button"
              onClick={() => petStatBuy(equipedPet, strengthBuy)}
            >
              Buy {Math.floor(equipedPet.statCost)}XP
            </button>
          </div>
          <div className="heroPetXpStatExchange__speed">
            Speed: {equipedPet.speed}
            <button className="heroPetXpStatExchange__Speed--button" onClick={() => petStatBuy(equipedPet, speedBuy)}>
              Buy {Math.floor(equipedPet.statCost)}XP
            </button>
          </div>
          <div className="heroPetXpStatExchange__luck">
            Luck: {equipedPet.luck}
            <button className="heroPetXpStatExchange__Luck--button" onClick={() => petStatBuy(equipedPet, luckBuy)}>
              Buy {Math.floor(Math.floor(equipedPet.statCost))}XP
            </button>
          </div>
        </div>
      );
