...
class HeroActionStore {
    ...
    patrolBattleStart = (location, levelMin, levelMax, MIA, FBIA) => {
        if (!location) {
            ...
            if (FBIA.length > 0 && randomNumber(0, 100) <= 1) {
                ...
                this.monsterHealth =
                    this.monster.health * this.monsterLevelMulti.multi;
                this.monsterStrength =
                    this.monster.strength * this.monsterLevelMulti.multi -
                    (this.allStores.heroStatsStore.constitution +
                        this.allStores.heroStatsStore.equipedHeroArmour);
                this.monsterSpeed =
                    this.monster.speed * this.monsterLevelMulti.multi;
                ...
                this.monsterMoneyDrop = Math.round(
                    randomNumber(this.monster.moneyMin, this.monster.moneyMax) *
                        this.monsterLevelMulti.multi
                );
                ...
                this.monsterLevelMulti =
                    levelChart[randomNumber(levelMin, levelMax)];
                this.monsterName = this.monster.name;
                this.monsterHealth =
                    this.monster.health * this.monsterLevelMulti.multi;
                this.monsterStrength =
    ...
    patrolBattleAttack = () => {
        ...
        if (this.monsterHealth <= 0) {
            ...
            this.allStores.heroStatsStore.heroAttackAmount >= this.monsterHealth
        ) {
            ...
            if (!this.underAttack) {
                this.monsterHealth =
                    this.monsterHealth -
                    this.allStores.heroStatsStore.heroAttackAmount;
    ...
    heroHealthInterval = () => {
        this.heroHealthCheckInterval = setInterval(
            this.heroHealthStatement,
            10
        );
    };
    ...
    heroPetInterval = () => {
        if (this.allStores.heroStatsStore.equipedPet.name) {
            this.petInterval = setInterval(
                this.heroPetAttack,
                1000 / this.allStores.heroStatsStore.equipedPet.speed
            );
        }
    };
    ...
    heroPetAttack = () => {
        this.monsterHealth =
            this.monsterHealth -
            this.allStores.heroStatsStore.equipedPet.strength;
    };

    heroMonsterAttackInterval = () => {
        this.monsterInterval = setInterval(
            this.monsterAttack,
            1000 / this.monsterSpeed
        );
    };
    ...
    monsterAttack = () => {
        this.allStores.heroStatsStore.health =
            this.allStores.heroStatsStore.health -
            Math.round(this.monsterStrength);
    };
    ...
    patrolLoss = () => {
        ...
        clearInterval(this.heroHealthCheckInterval);
        clearInterval(this.petInterval);
        this.allStores.countStore.money =
            this.allStores.countStore.heroMoney *= 0.9;
    ...
    lootDrops = () => {
        this.monsterDrop = lootDrop(
            this.monster,
            this.allStores.heroStatsStore.luck
        );
        for (let i = 0; i < this.monsterDrop.length; i++) {
            this.allStores.heroInventoryStore.inventoryPlacement(
                this.monsterDrop[i]
            );
        }
    ...
    skillScreen = () => {
        if (this.selectedActionArea !== <SkillScreen />) {
            this.selectedActionArea = <SkillScreen />;
        }
    };
    ...
    tanning = (hide) => {
        if (this.tanningActive === true) {
            ...
            this.rawHide = hide;
            this.tanTime =
                (hide.tanDifficulty / this.allStores.countStore.tanningLevel) *
                10;
    ...
    tanningProgress = (action) => {
        if (this.tanningProgressState === this.tanTime) {
            ...
            this.tanningProgressState =
                this.tanningProgressState +
                this.allStores.countStore.tanningLevel;
        } else {
            this.tanningProgressState++;
        }
    };
    ...
    tanningComplete = () => {
        const tanChance =
            this.rawHide.tanDifficulty - this.allStores.countStore.tanningLevel;
