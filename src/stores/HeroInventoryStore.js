  ...
  handleSell = (item, amount) => {
    ...
    if (this.heroItemsInv.some((el) => el.name === item.name) && item.type === "item") {
      if (amount === "one" && item.count === 1) {
        ...
        this.allStores.countStore.heroMoney = this.allStores.countStore.heroMoney + item.cost;
        ...
        const position = this.heroItemsInv.indexOf(item.name) + 1;
        this.heroItemsInv.splice(position, 1);
        this.allStores.countStore.heroMoney = this.allStores.countStore.heroMoney + item.cost * item.count;
      }
    }
    if (this.heroWeaponInv.some((el) => el.name === item.name) && item.type === "weapon") {
      if (amount === "one" && item.count === 1) {
